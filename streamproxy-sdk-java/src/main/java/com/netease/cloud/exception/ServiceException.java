 ...
 * Receiving an exception of this type indicates that the caller's request was correctly transmitted
 * to the service, but for some reason, the service was not able to process it, and returned an
 * error response instead.
 * <p>
 * ServiceException provides callers several pieces of information that can be used to obtain more
 * information about the error and why it occurred. In particular, the errorType field can be used
 * to determine if the caller's request was invalid, or the service encountered an error on the
...
public class ServiceException extends ClientException {
     ...
     * 
     * <p>
     * For example, if a client is using an invalid access key, the returned exception will indicate
     * that there is an error in the request the caller is sending. Retrying that same request will
     * *not* result in a successful response. The Client ErrorType indicates that there is a problem
     * in the request the user is sending (ex: incorrect access keys, invalid parameter value,
     * missing parameter, etc.), and that the caller must take some action to correct the request
     * before it should be resent. Client errors are typically associated an HTTP error code in the
     * 4xx range.
     * 
     * <p>
    ...
    /**
     * Constructs a new ServiceException with the specified message and exception indicating the
     * root cause.
     * 
     * @param message An error message describing what went wrong.
     ...
     * 
     * @return The request ID that uniquely identifies the service request the caller made.
     */
    ...
    /**
     * Returns a string summary of the details of this exception including the HTTP status code,
     * request ID, error code and error message.
     * 
     * @see java.lang.Throwable#toString()
     */
    @Override
    public String toString() {
        return "Status Code: " + getStatusCode() + ", " + "Service: " + getServiceName() + ", "
                + "Request ID: " + getRequestId() + ", " + "Error Code: " + getErrorCode() + ", "
                + "Error Message: " + getMessage() + ", " + "Error Resource: " + getResource();
    }
